...
@time begin # Measure the computation time
    ...
    #################### Initialization ####################
    ## Initialize number of Fourier coefficients (zero padding)
    N01 = 80 # Sequence truncation dimension, N0>=N
    N02 = 60 

    # Initialize parameters to define how large the finite domain is
    d1 = interval(pi) / interval(0.1)
    d2 = interval(pi) / interval(0.2)

    N1 = 80# Operator truncation dimension
    N2 = 60 
    nu1 = interval(1.1) # Used for computing aliasing error
    nu2 = interval(1.1) 
    Nfft1 = 512 # Should be powers of 2
    Nfft2 = 512 
    c = interval(0.8) # Wave speed

    #################### Newton ####################

    d1float = π / 0.1 # No need to work with intervals for improving the solution using Newton's method
    d2float = π / 0.2
    cfloat = 0.8

    fourier0float = CosFourier(N01, π / d1float) ⊗ CosFourier(N02, π / d2float)
    # Import 81x61 approximate solution in Fourier coefficients for SB
    u = load("useq_c08.jld2", "uNewton")

    ucoeff = reshape(coefficients(u), 81, 61)
    ...
    # When running the code with t defined as above in comments, we obtain a spectrum bigger than -0.4
    # Hence, we choose t=0.5 manually to improve results (cf. Theorem 6.3)
    t = interval(0.5) ### value of t for c = 0.8
